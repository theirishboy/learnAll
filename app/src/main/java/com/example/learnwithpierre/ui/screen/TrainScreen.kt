...
)
{
    ...
    if (popUpControl == 1) {
        BottomPopUp(modifier = modifier, OnContinue = { viewModel.nextQuestion() }, color = md_theme_light_primary)
    } else if(popUpControl == 2){
        BottomPopUp(modifier = modifier, OnContinue = { viewModel.nextQuestion() }, color =  md_theme_light_tertiary)
    }
...
@Composable
fun BottomPopUp(modifier: Modifier, OnContinue: () -> Unit,color: Color) {
    Popup(
        ...
        Column(
            modifier
                ...
                .fillMaxWidth()
                .fillMaxHeight(0.2F), horizontalAlignment = Alignment.CenterHorizontally) {
...
private fun BottomPopUpPreview(){
    BottomPopUp(Modifier,{}, Color.Red)
}